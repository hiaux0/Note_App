define(['jquery', 'context_menu_lib','inline_styles'], function($,cm,inSty) {
// start ======================================
let doc = document,
    grid = {},
    methods = {},
 ...
 */
    ...
    // add dynamic style to wrapper
        inSty.method.addInlineStyle(target,{
...
stylesheet.innerHTML = inSty.method.toStylesheet(manage_styles)
...
function listenToHighlight(ev) {
    ...
    if (!ev.path[0].parentNode.classList.hasOneOf("context-menu-item context-menu-list")) {
        ...
        if (ev.target.childNodes[0]) {
            changeDisplayedText = ev.target.childNodes[0].nodeValue;
        }
    }
};
